...
#define recv_count_max 3			//????????????§ß????????????????
float rotated_vy;
float rotated_vx;

uint8_t mecanum_recv_flag = 1;		//?????????¦Ë?????????????????????
uint16_t mecanum_recv_count = 0;	//????¦Ä????????????
...
void Mecanum_data_Send(UART_HandleTypeDef* huart_x, unsigned char* pBuffer){
	mecanum_Send.X_Integ = (int32_t)(vT13.Chassis_X_Integ * 1000 + rc_Ctrl.Chassis_X_Integ * 2000);
	mecanum_Send.Y_Integ = (int32_t)(vT13.Chassis_Y_Integ * 1000 + rc_Ctrl.Chassis_Y_Integ * 2000);
	mecanum_Send.vt13_mouse_x = (vT13.mouse.x + rc_Ctrl.mouse.x) * 3 + ((vT13.rc.ch2 - 1024) + (rc_Ctrl.rc.ch2 - 1024)) * 0.3;
